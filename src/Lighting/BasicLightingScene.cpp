...
// A Basic Lighting Scene is set up for further experiments
...
int main()
{
    ...
    // Load Object Shader
    Shader objectShader("shaders/BasicLightingScene.vert", "shaders/BasicLightingScene.frag");
    objectShader.use();
    objectShader.setVec3("objectColor", glm::vec3(1.0f, 0.5f, 0.3f));
    objectShader.setVec3("lightColor", glm::vec3(1.0f, 1.0f, 1.0f));

    // A set of vertices to describe a cube
    float vertices[] = {
    ...
    // Vertex Buffer Objects(VBO) are used to pass vertices to GPU for the vertex objectShader
    unsigned int VBO;
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        lightShaderModel = glm::translate(lightShaderModel, glm::vec3(1.2f, 0.5f, 1.0f));
        lightShaderModel = glm::scale(lightShaderModel, glm::vec3(0.1f));
        ...
        for (int i = 0; i < 2; ++i) {
            ...
            if (i == 0) model = glm::scale(model, glm::vec3(1.3f));
